...
fix_unqualified_fns_bulk = function(
  ...
  for (packge in packages) {
    ...
    if (length(functions) > 0) {
      ...
      # stringr::str_detect(c("no_testfn(sdf)","another_testfn(sdf)","old.testfn(sdf)", "list$testfn(sdf)", "list@testfn(sdf)"), functionRegex)
      ...
      for (i in 1:nrow(files)) {
        ...
        # fix the unqualifed
        files$content[[i]] = file$content %>%
          stringr::str_replace_all(functionRegex, replacement)
        # which function matched we need to know which one was matched
        tmp = stringr::str_match_all(file$content, functionRegex) %>%
          purrr::map(~ .x[, 3]) %>%
          unlist()
        if (is.null(tmp)) {
          tmp = character()
  ...
  if (any(files$changed)) {
    ...
    if (fixme %in% c(1)) {
      ...
      nsMissing = files %>%
        ...
        dplyr::filter(!(pkg %in% c(imports, suggests))) %>%
        dplyr::pull(pkg) %>%
        unique()

      if (length(nsMissing) > 0) {
        message(
          "fixing missing DESCRIPTION imports:\n",
          paste0(nsMissing, collapse = ",")
        )

        desc = desc::desc(file = description_file)
        x = apply(nsMissing, desc$set_dep, type = "Imports")
        .write_unsafe(desc, file = description_file)
      }
    }
  }
...
.package_map = function(
  ...
  packages = forcats::as_factor(unique(c(
    cur_package,
    "base",
    "utils",
    dplyr::intersect(packages, tmp),
    packages
  )))
  nsqualifier = ifelse(
    packages %in% c(cur_package, "base"),
    "",
    sprintf("%s::", packages)
  )
  ...
  packageMap = tibble::tibble(
    ...
    dplyr::mutate(
      function_name = list(getNamespaceExports(as.character(package)))
    ) %>%
...
# content <- c("data <- read_csv('file.csv')",
#              "filtered <- filter(data, x > 5)",
#              "result <- dplyr::mutate(data, y = x * 2)",
#              "# result filter (data, y = x * 2)",
#              "result <- dplyr::mutate(data, y = x * 2)")
# package_name <- "dplyr"
...
.process_content = function(content, package_map) {
  ...
  for (packge in packages) {
    ...
    if (length(functions) > 0) {
      ...
      # # functionNames = "testfn
      # # stringr::str_detect(c("testfn(sdf)","testfn(sdf)","+testfn(sdf)","+testfn (sdf)"), functionRegex)
      # # stringr::str_detect(c("no_testfn(sdf)","another_testfn(sdf)","old.testfn(sdf)","any # commented out testfn(sdf)"), functionRegex)
      # functionRegex = paste0("(?<![:a-zA-Z0-9\\._\\\\])(",functionNames,")\\(")
      # replacement = paste0(packge,"::\\2(")
      # # c = files$content.old[[1]]
      # content = content %>% stringr::str_replace_all(functionRegex, replacement)
      #
      # # content = content %>%
      # #   lapply(stringr::str_split_1,"#") %>%
      # #   lapply(\(x) x[[1]] = x[[1]] %>% stringr::str_replace_all(functionRegex, replacement) ) %>%
      ...
      content = tmp$modified_content
      matches = matches %>% dplyr::bind_rows(tmp$summary)
    }
  }

...
update_standalone = function(
  ...
  yml = .merge_standalone(
    content, path, repo=repo, license = license, dependencies = dependencies
  )
  yml = c("# ---", paste0("# ",unlist(stringr::str_split(trimws(yaml::as.yaml(yml)),"\\n"))), "# ---")
  ...
  if (!identical(content,new_content)) {
    print(diffobj::diffChr(content, new_content, interactive = FALSE, mode="sidebyside"))
    proceed = utils::askYesNo("Update editor with suggested changes?",default = TRUE)
    if (proceed) rstudioapi::setDocumentContents(text = paste0(new_content, collapse="\n"), context$id)
  } else {
...
.merge_standalone = function(
  ...
  if (updated || is.null(yaml$`last-updated`)) yaml$`last-updated` = as.character(Sys.Date())
  if (!is.null(repo)) yaml$repo = repo
  if (!is.null(licence)) yaml$license = license
  if (is.null(yaml$license)) yaml$license = "https://unlicense.org"
  if (is.null(yaml$repo)) {
    yaml$repo = "<ADD REPO DETAILS>"
  }
  yaml$dependencies = sort(unique(c(yaml$dependencies,dependencies)))
  yaml$imports = sort(unique(c(yaml$imports,imports)))
  yaml$imports = yaml$imports[yaml$imports != ""]
  yaml = yaml[intersect(c("repo","file","last-updated","license","dependencies","imports"),names(yaml))]
  return(yaml)
}
...
.local_imports = function(content, path) {
  ...
  if (length(tmp$matches$pkg) > 0) message("unqualified functions found. Please run `fix_unqualified_fns()` to resolve.",call. = FALSE)

...
#' with dependencies managed by `renv`, or not mamanged at all. If an analysis
...
use_standalone = function (repo_spec, file = NULL, ref = NULL, host = NULL) {
  ...
  if (inherits(tmp,"try-error")) {
    ...
    lines = tryCatch( readr::read_lines(fs::path(wd,".Rprofile")), error = function(e) character())
    ...
    for (i in seq_len(nrow(imports))) {
      ...
      if (renv_mode) {    
        ...
        lines = lines[!stringr::str_detect(lines, sprintf("# depends on %s",import$pkg))]
        lines = c(
          sprintf("if (!requireNamespace(\"%s\")) message(\"Package `%s` must be installed.\") # depends on %s",
                  import$pkg, 
...
#' a local master standalone file, which is assumed to be in a sibling project
#' directory. This allows for changes to be propagated back to the master when
#' in active development.
#'
#' @return nothing used for side effects
#' @export
sync_standalone_to_master = function() {
  if (requireNamespace("rstudioapi",quietly = TRUE) && requireNamespace("diffobj",quietly = TRUE)) {
    ...
    new_content = utils::tail(content, -min(which(content == "# ---")))
    source_path = 
      fs::path_abs(
        fs::path("..",
            stringr::str_extract(yaml$repo,"[^/]+/([^/]+)",group=1),
            "R",
            yaml$file
        ), start = wd         
      )
    if (!fs::file_exists(source_path)) stop("Cannot find original standalone at:",source_path)
    ...
    if (identical(new_content, old_content)) {
      ...
      if (!.punkmode()) {
        print(diffobj::diffChr(old_content,new_content, interactive = FALSE, mode="sidebyside"))
        proceed = utils::askYesNo("Update source standalone file with suggested changes?",default = TRUE)
      } else {
        proceed = TRUE
      }
      
      if (proceed) {
        new_content[stringr::str_starts(new_content,"# last-updated:")] = sprintf("# last-updated: %s", format(Sys.Date()))
...
#TOOD: push_standalone & pull_standalone
# gh::gh 

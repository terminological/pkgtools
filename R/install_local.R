...
install_local = function(
  ...
  devtools::document(pkg$path, quiet = quiet)
  ...
  if (renv_mode) {
    ...
    local_path = fs::path(
      renv_local,
      pkg$package,
      sprintf("%s_%s.tar.gz", pkg$package, pkg$version)
    )
    ...
    # make sure renv does not .gitignore its local directory
    readr::read_lines(fs::path(wd, "renv", ".gitignore")) %>%
      purrr::discard(~ .x == "local/") %>%
      readr::write_lines(fs::path(wd, "renv", ".gitignore"))
  } else {
